...
def run_benchmarks(robot_id, obstacles, num_runs=20):
    print(f"--- Running {num_runs} Automated Benchmarks ---")
    
    baseline_metrics = {'time': [], 'nodes': [], 'path_length': [], 'success': 0}
    enhanced_metrics = {'time': [], 'nodes': [], 'path_length': [], 'success': 0}

    for run in range(num_runs):
        # 1. Randomize Start and Goal 
        start_q = [random.uniform(LOWER_LIMITS[j], UPPER_LIMITS[j]) for j in range(7)]
        goal_q = [random.uniform(LOWER_LIMITS[j], UPPER_LIMITS[j]) for j in range(7)]
        
        # Initialize Planner
        rrt_planner = RRT(robot_id, obstacles, start_q, goal_q)
        ...
        if raw_path is not None:
            baseline_metrics['success'] += 1
            baseline_metrics['time'].append(calc_time)
            baseline_metrics['nodes'].append(len(rrt_planner.node_list))
            
            # Calculate task-space path length
            length = sum(np.linalg.norm(np.array(raw_path[i]) - np.array(raw_path[i-1])) for i in range(1, len(raw_path)))
            baseline_metrics['path_length'].append(length)

            # --- ENHANCED TEST (Phase B: Smoothing) ---
            smooth_start_time = time.time()
            smooth_path = shortcut_path(robot_id, raw_path, obstacles)
            smooth_calc_time = (time.time() - smooth_start_time) + calc_time
            
            enhanced_metrics['success'] += 1
            enhanced_metrics['time'].append(smooth_calc_time)
            enhanced_metrics['nodes'].append(len(smooth_path))
            
            smooth_length = sum(np.linalg.norm(np.array(smooth_path[i]) - np.array(smooth_path[i-1])) for i in range(1, len(smooth_path)))
            enhanced_metrics['path_length'].append(smooth_length)

    # Print the Markdown Table directly to the console
    print("\n| Metric | Baseline APF-RRT | Enhanced (Optimization-Smoothed) |")
    print("| :--- | :--- | :--- |")
    print(f"| Success Rate (N={num_runs}) | {baseline_metrics['success']/num_runs * 100}% | {enhanced_metrics['success']/num_runs * 100}% |")
    print(f"| Avg Computation Time | {np.mean(baseline_metrics['time']):.3f} sec | {np.mean(enhanced_metrics['time']):.3f} sec |")
    print(f"| Avg Path Length (rad) | {np.mean(baseline_metrics['path_length']):.2f} | {np.mean(enhanced_metrics['path_length']):.2f} |")
    print(f"| Avg Node Count | {int(np.mean(baseline_metrics['nodes']))} nodes | {int(np.mean(enhanced_metrics['nodes']))} nodes |")
    ...
    with open("benchmark_results.csv", "w", newline="") as f:
        ...
        writer.writerow(["Success Rate",
                        baseline_metrics['success']/num_runs,
                        enhanced_metrics['success']/num_runs])

        writer.writerow(["Avg Time",
                        np.mean(baseline_metrics['time']),
                        np.mean(enhanced_metrics['time'])])

        writer.writerow(["Avg Path Length",
                        np.mean(baseline_metrics['path_length']),
                        np.mean(enhanced_metrics['path_length'])])

        writer.writerow(["Avg Nodes",
                        np.mean(baseline_metrics['nodes']),
                        np.mean(enhanced_metrics['nodes'])])

    print("✅ Results saved to benchmark_results.csv")

    plot_metrics(baseline_metrics, enhanced_metrics)
# To use this, comment out your FSM while True loop and call:
# run_benchmarks(robot_id, obstacles, num_runs=20)

def plot_metrics(baseline, enhanced):

    labels = ["Time", "Path Length", "Nodes"]
    baseline_vals = [
        np.mean(baseline['time']),
        np.mean(baseline['path_length']),
        np.mean(baseline['nodes'])
    ]
    enhanced_vals = [
        np.mean(enhanced['time']),
        np.mean(enhanced['path_length']),
        np.mean(enhanced['nodes'])
    ]

    x = np.arange(len(labels))

    plt.figure()
    plt.bar(x - 0.2, baseline_vals, 0.4, label='Baseline')
    plt.bar(x + 0.2, enhanced_vals, 0.4, label='Enhanced')
...
def plot_rrt_tree_3d(robot_id, rrt_planner, final_path):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    
    # Helper to convert joint angles to End-Effector XYZ
    def get_ee_xyz(q):
        state_id = p.saveState()   # ✅ SAVE STATE
        for i in range(7):
            p.resetJointState(robot_id, i, q[i])
        pos = p.getLinkState(robot_id, 11)[0]
        p.restoreState(state_id)   # ✅ RESTORE
        p.removeState(state_id)
        return pos
    
    # Plot the full exploratory tree (Grey)
    for node in rrt_planner.node_list:
        if node.parent is not None:
            p1 = get_ee_xyz(node.q)
            p2 = get_ee_xyz(node.parent.q)
            ax.plot([p1[0], p2[0]], [p1[1], p2[1]], [p1[2], p2[2]], color='grey', alpha=0.3)
            
    # Plot the final smoothed path (Blue)
    if final_path:
        path_xyz = [get_ee_xyz(q) for q in final_path]
        xs, ys, zs = zip(*path_xyz)
        ax.plot(xs, ys, zs, color='blue', linewidth=3, label='Smoothed Path')
        
    ax.set_title("3D Task Space RRT Tree & Path")
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_zlabel("Z (m)")
    plt.legend()
    #plt.show()
    plt.savefig("rrt_path_visualization.png", dpi=300)
    plt.close()   # ✅ IMPORTANT
    print("✅ Plot saved as rrt_path_visualization.png")

if __name__ == "__main__":
    # --- CHANGED: Run in Headless mode (gui=False) for blazing fast benchmarking ---
    plane_id, robot_id = start_simulation(gui=False) 
    ...
    # =================================================================
    # TASK 2: DATA GENERATION & VISUALIZATION
    # =================================================================
    
    # 1. Generate the Comparative Analysis Table (20 runs)
    run_benchmarks(robot_id, obstacles, num_runs=20)
    
    # 2. Generate the 3D Tree Plot (Single explicit run)
    print("\n--- Generating 3D Plot for Documentation ---")
    start_q = REST_POSES
    
    # Create a challenging arbitrary goal across the canopy
    goal_q = p.calculateInverseKinematics(
        robot_id, 11, [0.5, 0.4, 0.1], 
        lowerLimits=LOWER_LIMITS, upperLimits=UPPER_LIMITS, 
        jointRanges=JOINT_RANGES, restPoses=REST_POSES, maxNumIterations=200)
    ...
    if raw_path:
        smooth_path = shortcut_path(robot_id, raw_path, obstacles)
        plot_rrt_tree_3d(robot_id, rrt_planner, smooth_path)
    else:
        print("Could not find a path for the visualization run.")

    # =================================================================
    # TASK 1: MAIN PICK-AND-PLACE FSM (Commented out for benchmarking)
    # =================================================================
    ...
    #             time.sleep(0.5)
    #
    #     # ... (The rest of your FSM remains safely commented out here) ...